...
def plot_finite_size_scaling(
    ...
    p_inf,
    alpha,
    output_path="figures/finite_size_scaling.png"
):
    ...
    plt.plot(
        ...
        p_inf + N_fit**(-alpha),
        color='red',
        label=f"Fit: p_inf={p_inf:.3f}, alpha={alpha:.3f}"
    )
...
def plot_susceptibility(
    link_probs,
    susceptibility,
    output_path="figures/susceptibility_curve.png"
):
    plt.figure(figsize=(6,4))
...
if __name__ == "__main__":
    ...
    df_fs = pd.read_csv(
        "data/processed/critical_thresholds.csv"
    )
    ...
    df_susc = pd.read_csv(
        "data/processed/susceptibility.csv"
    )

    link_probs = df_susc["link_retention_prob"]
    susceptibility = df_susc["susceptibility"]
